...
DFS 스패닝 트리를 만들면 원래 그래프의 간선들을 분류할 수 있다.
...
DFS 중에 이미 방문했지만 아직 탐색이 끝나지 않은 정점(스택에 있는 조상)을 다시 만나면,
그것이 역방향 간선이고 곧 사이클이다.
...
- 역방향 간선이 하나도 없으면 → 사이클 없음 (트리)
- 역방향 간선이 하나라도 있으면 → 사이클 존재
...
**disc** (discovery time): DFS에서 해당 정점을 **처음 방문한 순서**

**low**: 해당 정점에서 **자손들과 역방향 간선을 통해 도달할 수 있는 가장 작은 disc 값**
...
low는 DFS 재귀가 돌아오면서(return 할 때) 자식 → 부모로 **전파**되어 계산된다.

### 단절점 (Articulation Point)

**조건: `low[child] >= disc[node]`**

의미: 자식 subtree의 비상 통로(역방향 간선)가 node 위까지 못 올라감
→ node를 제거하면 자식 subtree가 고립 → **단절점**

루트 예외: 루트는 위 조건 대신, **DFS 트리에서 자식이 2개 이상**이면 단절점

### 단절선 (Bridge)

**조건: `low[child] > disc[node]`**

의미: 자식 subtree의 비상 통로가 node까지도 못 올라감
